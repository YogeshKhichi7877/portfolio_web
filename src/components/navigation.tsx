...
import { ArrowUpRight, Command, List, Search, X } from 'lucide-react';
...
export function Navigation() {
  ...
  useEffect(() => {
    ...
    const updateChapter = () => {
      ...
      for (const chapter of chapters) {
        ...
        if (section && section.getBoundingClientRect().top <= window.innerHeight * .32) current = chapter.id;
      }
      setActive(current);
    };
    const schedule = () => { if (!frame) frame = requestAnimationFrame(updateChapter); };
    window.addEventListener('scroll', schedule, { passive: true });
    window.addEventListener('resize', schedule);
    schedule();
    return () => { window.removeEventListener('keydown', handleKey); window.removeEventListener('scroll', schedule); window.removeEventListener('resize', schedule); cancelAnimationFrame(frame); };
  }, []);
  ...
  return <>
    ...
    <nav className="chapter-rail" aria-label="Chapter navigation"><div className="rail-heading" aria-hidden="true"><List size={16}/><span>Chapters</span></div>{chapters.map((c, i) => <a key={c.id} href={`#${c.id}`} className={active === c.id ? 'active' : ''} aria-label={`Chapter ${String(i + 1).padStart(2, '0')}: ${c.label}`} aria-current={active === c.id ? 'location' : undefined}><span className="rail-marker" aria-hidden="true"/><span className="rail-label"><small>{String(i + 1).padStart(2, '0')}</small>{c.label}</span></a>)}</nav>
