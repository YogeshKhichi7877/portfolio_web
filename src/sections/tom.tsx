...
export function Tom() {
  return <section id="tom" className="chapter tom section-pad" data-stage="listen"><ChapterLabel number="05" title="TOM / THE INTELLIGENCE LAYER"/>
    ...
    <div className="tom-scene tom-loom"><div className="tom-system"><div className="tom-system-head"><span className="eyebrow">SIGNAL STUDY / 001</span><span className="tom-phase">01 / LISTEN</span></div>
      <figure className="tom-signal" data-cursor="FOLLOW THE SIGNAL"><canvas className="signal-canvas" width={700} height={440} aria-hidden="true"/><svg viewBox="0 0 700 440" aria-hidden="true"><defs><linearGradient id="signal-ink" x1="0" y1="0" x2="1" y2="0"><stop offset="0" stopColor="#72a8c5"/><stop offset=".5" stopColor="#b9f7d2"/><stop offset="1" stopColor="#77c0b5"/></linearGradient></defs><g className="signal-registration">{[60,160,260,360,460,560,660].map(x=><path key={x} d={`M${x} 28v8M${x} 402v8`}/>)}<path d="M24 220H38M662 220H676"/></g><g className="signal-strands">{Array.from({length:SIGNAL_THREADS},(_,i)=><path key={i} className="signal-strand" d={signalPath(signalPoints(0,i))}/>)}</g><line className="signal-playhead" x1="350" x2="350" y1="50" y2="390"/><text x="45" y="30">INPUT</text><text x="655" y="30" textAnchor="end">OUTCOME</text><text x="350" y="414" textAnchor="middle">LOCAL / PRIVATE / CONNECTED</text></svg><figcaption><span className="nucleus-state">VOICE / INPUT</span><span>SCROLL TO TRANSFORM ↓</span></figcaption></figure>
